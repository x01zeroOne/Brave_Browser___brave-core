...
TEST_F(KeyringServiceUnitTest, SetSelectedAccount) {
  ...
  {
    absl::optional<std::string> account = ImportFilecoinAccount(
    ...
    EXPECT_EQ(*account, "t1h4n7rphclbmwyjcp6jrdiwlfcuwbroxy3jvg33q");
    EXPECT_TRUE(SetSelectedAccount(&service, &observer, account.value(),
                                   mojom::CoinType::FIL));
    EXPECT_EQ(account.value(),
  ...
  // Can set Solana account
  {
    ...
    absl::optional<std::string> account = ImportAccount(
        ...
        mojom::CoinType::SOL);
    ASSERT_TRUE(account.has_value());
    EXPECT_EQ(*account, "C5ukMV73nk32h52MjxtnZXTrrr7rupD9CTDDRnYYDRYQ");
    EXPECT_TRUE(SetSelectedAccount(&service, &observer, account.value(),
                                   mojom::CoinType::SOL));
    EXPECT_EQ(account.value(),
