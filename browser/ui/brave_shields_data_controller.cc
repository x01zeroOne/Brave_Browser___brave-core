...
void BraveShieldsDataController::AllowScriptsOnce(
  ...
  if (observer) {
    std::transform(
        ...
        std::inserter(resource_list_allowed_once_js_,
                      resource_list_allowed_once_js_.begin()),
        [](const std::string& origin) -> GURL { return GURL(origin); });
...
void BraveShieldsDataController::HandleItemAllowedOnce(
  ...
  if (allowed_once_type == kJavaScript) {
    resource_list_allowed_once_js_.insert(subres);
  }
