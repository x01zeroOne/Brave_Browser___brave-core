...
SyncDeviceFormFactor const SyncDeviceFormFactorUnknown = static_cast<NSInteger>(
    syncer::DeviceInfo::FormFactor::kUnknown);
SyncDeviceFormFactor const SyncDeviceFormFactorDesktop = static_cast<NSInteger>(
    syncer::DeviceInfo::FormFactor::kDesktop);
SyncDeviceFormFactor const SyncDeviceFormFactorPhone = static_cast<NSInteger>(
    syncer::DeviceInfo::FormFactor::kPhone);
SyncDeviceFormFactor const SyncDeviceFormFactorTablet = static_cast<NSInteger>(
...
- (void)addTab:(const sessions::SessionTab&)session_tab
  ...
  if (session_tab.navigations.size() > 0) {
    ...
    // Store the tab
    IOSOpenDistantTab* distant_tab =
      [[IOSOpenDistantTab alloc] initWithURL:net::NSURLWithGURL(virtual_url)
...
- (NSArray<IOSOpenDistantSession*>*)getSyncedSessions {
  ...
  // but modified to allow us to retrieve `device_type` via GetDeviceFormFactor()
  ...
  if (open_tabs_delegate) {
    ...
    for (const auto* session : sessions) {
      ...
      IOSOpenDistantSession* distant_session = [[IOSOpenDistantSession alloc]
          ...
          deviceFormFactor:static_cast<SyncDeviceFormFactor>(session->GetDeviceFormFactor())];
      
      // Add tabs to the distant session
      std::vector<const sessions::SessionTab*> open_tabs;
      open_tabs_delegate->GetForeignSessionTabs(session->session_tag, &open_tabs);
    ...
    [sessions_list sortedArrayUsingComparator:
         ^NSComparisonResult(IOSOpenDistantSession*a, IOSOpenDistantSession* b) {
