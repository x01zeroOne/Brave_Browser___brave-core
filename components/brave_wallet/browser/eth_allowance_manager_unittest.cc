...
constexpr base::StringPiece eth_allowance_error_response = R"({
                  "error": {
                    "code": -32000,
                    "message": "requested too many blocks from 0 to 27842567, maximum is set to 2048"
                  },
                  "id": 1,
                  "jsonrpc": "2.0"
                })";
      ...
      eth_allowance_manager_->DiscoverEthAllowancesOnAllSupportedChains(
          base::BindLambdaForTesting(
              [&](std::vector<mojom::AllowanceInfoPtr> allowances,
                  mojom::ProviderError error, const std::string& descr) {
                callback_count++;
                allowances_validation.Run(allowances, error, descr);
                if (callback_count == eth_allowance_completed_call_count) {
                  run_loop.Quit();
                }
              }));
    }
...
TEST_F(EthAllowanceManagerUnitTest, BreakAllowanceDiscoveringIfTokenListEmpty) {
  ...
  eth_allowance_manager_->DiscoverEthAllowancesOnAllSupportedChains(
      base::BindLambdaForTesting(
          [&](std::vector<mojom::AllowanceInfoPtr> allowances,
              mojom::ProviderError error, const std::string& descr) {
            ASSERT_EQ(error, mojom::ProviderError::kInternalError);
...
TEST_F(EthAllowanceManagerUnitTest, AllowancesLoading) {
  ...
  auto allowances_validation = base::BindLambdaForTesting(
      [&expected_allowances](
          ...
          const mojom::ProviderError& error, const std::string& descr) {
        if (error != mojom::ProviderError::kSuccess) {
          ASSERT_TRUE(allowances.empty());
          return;
        } else if (error == mojom::ProviderError::kInternalError) {
          ASSERT_EQ(
              l10n_util::GetStringUTF8(IDS_WALLET_ALREADY_IN_PROGRESS_ERROR),
              descr);
        }
        ASSERT_FALSE(allowances.empty());
...
TEST_F(EthAllowanceManagerUnitTest, AllowancesIgnorePendingBlocks) {
  ...
  auto allowances_validation = base::BindLambdaForTesting(
      [](const std::vector<mojom::AllowanceInfoPtr>& allowances,
         const mojom::ProviderError& error, const std::string& descr) {
        ASSERT_EQ(mojom::ProviderError::kSuccess, error);
        ASSERT_TRUE(descr.empty());
...
TEST_F(EthAllowanceManagerUnitTest, NoAllowancesLoadedForSkippedNetwork) {
  ...
  auto allowances_validation = base::BindLambdaForTesting(
      [](const std::vector<mojom::AllowanceInfoPtr>& allowances,
         const mojom::ProviderError& error, const std::string& descr) {
        if (error == mojom::ProviderError::kInternalError) {
          ASSERT_EQ(
              l10n_util::GetStringUTF8(IDS_WALLET_ALREADY_IN_PROGRESS_ERROR),
              descr);
        } else {
