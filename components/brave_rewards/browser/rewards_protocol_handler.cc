...
void LoadRewardsURL(
  ...
  const auto gemini_production_host = gemini_production_oauth_url.host_piece();
  const auto gemini_sandbox_host = gemini_sandbox_oauth_url.host_piece();
  const auto uphold_production_host = uphold_production_oauth_url.host_piece();
  const auto uphold_sandbox_host = uphold_sandbox_oauth_url.host_piece();

  const base::StringPiece kAllowedDomains[] = {
      bitflyer_production_host,  // bitFlyer production
      bitflyer_sandbox_host,     // bitFlyer sandbox
      gemini_production_host,    // Gemini production
      gemini_sandbox_host,       // Gemini sandbox
      uphold_production_host,    // Uphold production
      uphold_sandbox_host        // Uphold sandbox
  };
