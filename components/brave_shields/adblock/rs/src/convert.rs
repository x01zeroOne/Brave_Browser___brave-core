...
use crate::ffi::{
    BlockerDebugInfo, BlockerResult, FilterListMetadata, OptionalString, RegexDebugEntry,
    RegexManagerDiscardPolicy,
};
use adblock::blocker::{
    BlockerDebugInfo as InnerBlockerDebugInfo, BlockerResult as InnerBlockerResult,
};
use adblock::lists::FilterListMetadata as InnerFilterListMetadata;
...
impl From<InnerFilterListMetadata> for FilterListMetadata {
    fn from(metadata: InnerFilterListMetadata) -> Self {
        Self { homepage: metadata.homepage.into(), title: metadata.title.into() }
    }
}
